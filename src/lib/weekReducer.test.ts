...
import { createWeekEntry } from "./weekPolicy";
import { rosterWeekReducer } from "./weekReducer";
...
void describe("roster week reducer", () => {
   ...
   void it("replaces changed roster data after a successful fetch", () => {
      const originalClass = week.classes[0];
      assert.ok(originalClass);
      const changed = createWeek({ classes: [{ ...originalClass, room: "B202" }] });
      const next = rosterWeekReducer({ 0: createWeekEntry(week) }, { type: "fetch-succeeded", weeks: [changed] });
      const nextEntry = next[0];
      assert.ok(nextEntry);

      assert.equal(nextEntry.data?.classes[0]?.room, "B202");
